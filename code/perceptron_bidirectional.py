...
Usage: python perceptron_bidirectional.py NITERATIONS
...
from evaluation_bidirectional import Eval
...
class Perceptron:
    def __init__(self, train_docs, train_labels, classes, MAX_ITERATIONS=100, dev_docs=None, dev_labels=None):
...
if __name__ == "__main__":
    ...
    print(len(X_train_docs), 'training docs with',
        sum(len(d) for d in X_train_docs)/len(X_train_docs), 'percepts on avg', file=sys.stderr)

    X_val_docs = load_featurized_docs(X_val)
    print(len(X_val_docs), 'dev docs with',
        sum(len(d) for d in X_val_docs)/len(X_val_docs), 'percepts on avg', file=sys.stderr)

    X_test_docs = load_featurized_docs(X_test)
    print(len(X_test_docs), 'test docs with',
        sum(len(d) for d in X_test_docs)/len(X_test_docs), 'percepts on avg', file=sys.stderr)

    ptron = Perceptron(X_train_docs, y_train, c_classes, MAX_ITERATIONS=niters, dev_docs=X_val_docs, dev_labels=y_val)
